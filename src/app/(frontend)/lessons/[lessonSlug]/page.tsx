...
import { LessonSidebar, type NavigationChapter } from '@/components/LessonSidebar'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!lesson) return {}
  const image = relation<Media>(lesson.meta?.image ?? lesson.featuredImage ?? 0)
  return {
    ...
    openGraph: { images: image?.url ? [image.url] : undefined, type: 'article' },
  }
}
...
export default async function LessonPage({ params }: Props) {
  ...
  const navigation: NavigationChapter[] = await Promise.all(chapters.map(async (item) => ({ chapter: item, lessons: await getLessons(item.id) })))
  ...
  return (
    <div className="lesson-shell">
      ...
      <div className="lesson-main">
        <Breadcrumbs items={[{ href: '/subjects', label: 'Subjects' }, { href: `/subjects/${subject.slug}`, label: subject.title }, { href: `/subjects/${subject.slug}/${chapter.slug}`, label: chapter.title }, { label: lesson.title }]} />
        <LiveLesson initialData={lesson} />
        <nav className="lesson-pager" aria-label="Lesson pagination">
          {previous ? <Link href={`/lessons/${previous.slug}`}><small>Previous</small><strong>&larr; {previous.title}</strong></Link> : <span />}
          {next ? <Link href={`/lessons/${next.slug}`}><small>Next</small><strong>{next.title} &rarr;</strong></Link> : <span />}
        </nav>
      </div>
      <aside className="toc"><details open><summary>On this page</summary>{toc.length ? <nav aria-label="On this page"><ol>{toc.map((item) => <li className={`level-${item.level}`} key={item.id}><a href={`#${item.id}`}>{item.text}</a></li>)}</ol></nav> : <p>No sections</p>}</details></aside>
