...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!subject) return {};
  return {
    title: subject.meta?.title ?? subject.title,
    description: subject.meta?.description ?? subject.description,
  };
}
...
export default async function SubjectPage({ params }: Props) {
  ...
  const lessons = await Promise.all(
    chapters.map((chapter) => getLessons(chapter.id)),
  );
        ...
        {chapters.map((chapter, index) => (
          ...
          >
            ...
            <span>
              ...
              <small>
                {chapter.description} · {lessons[index]?.length ?? 0} lessons
              </small>
            </span>
