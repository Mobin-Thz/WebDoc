...
const children = (node: LexicalNode, headingIDs: string[]): ReactNode =>
  (node.children ?? []).map((child, index) => (
    <Fragment key={index}>{renderNode(child, headingIDs)}</Fragment>
  ));

const renderText = (node: LexicalNode) => {
  let value: ReactNode = node.text ?? "";
  const format = Number(
    (node as LexicalNode & { format?: number }).format ?? 0,
  );
...
const renderNode = (node: LexicalNode, headingIDs: string[]): ReactNode => {
  ...
  if (node.type === "quote")
    return <blockquote>{children(node, headingIDs)}</blockquote>;
  if (node.type === "list") {
    const list = children(node, headingIDs);
    return (node as LexicalNode & { listType?: string }).listType ===
      "number" ? (
      <ol>{list}</ol>
    ) : (
      <ul>{list}</ul>
  ...
  if (node.type === "link" || node.type === "autolink") {
    ...
    const linked = fields.doc as
      | {
          relationTo?: string;
          value?: { slug?: string; subject?: { slug?: string } | number };
        }
      | undefined;
  ...
  if (node.type === "block") {
    ...
    if (fields.blockType === "code" && typeof fields.code === "string") {
      return (
        <CodeSnippet
          code={fields.code}
          language={
            typeof fields.language === "string" ? fields.language : "text"
          }
        />
      );
    }
      ...
      return id ? (
        <div className="video">
          <iframe
            ...
            title={
              typeof fields.title === "string" ? fields.title : "YouTube video"
            }
            allowFullScreen
            loading="lazy"
          />
    ...
    if (fields.blockType === "callout" && typeof fields.text === "string") {
      const kind = ["info", "warning", "tip"].includes(String(fields.kind))
        ? String(fields.kind)
        : "info";
      return (
        <Alert className={cn("callout", kind)}>
          <AlertTitle>{kind}</AlertTitle>
          <AlertDescription>{fields.text}</AlertDescription>
        </Alert>
      );
    }
  }
...
export function RichText({ content }: { content: LexicalContent }) {
  ...
  return (
    <div className="rich-text">{children(content.root ?? {}, headingIDs)}</div>
  );
}
