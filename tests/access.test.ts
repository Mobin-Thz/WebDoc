...
import {
  adminsOnly,
  publishedChapterOrAdmin,
  publishedLessonOrAdmin,
  publishedOrAdmin,
} from "@/access";

const call = (access: typeof adminsOnly, user: unknown = null) =>
...
describe("CMS access rules", () => {
  ...
  it("limits public reads to published documents and ancestors", () => {
    expect(call(publishedOrAdmin)).toEqual({
      _status: { equals: "published" },
    });
    expect(call(publishedChapterOrAdmin)).toMatchObject({
      and: expect.any(Array),
    });
    expect(call(publishedLessonOrAdmin)).toMatchObject({
      and: expect.any(Array),
    });
