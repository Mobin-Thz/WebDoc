...
const content = {
  root: {
    children: [
      ...
      {
        type: "paragraph",
        children: [{ type: "text", text: "A short lesson." }],
      },
      {
        type: "heading",
        tag: "h3",
...
describe("content utilities", () => {
  it("extracts text and estimates at least one minute", () => {
    expect(lexicalToText(content)).toBe(
      "Hello World A short lesson. Hello World",
    );
  ...
  it("normalizes slugs", () =>
    expect(slugify("  Linux & Files  ")).toBe("linux-files"));
});
...
describe("YouTube validation", () => {
  it("accepts supported URLs and rejects arbitrary embeds", () => {
    ...
    expect(getYouTubeID("https://www.youtube.com/watch?v=abc123")).toBe(
      "abc123",
    );
